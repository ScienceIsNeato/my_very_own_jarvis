...
def process_audio(
    ...
    Returns:
        Optional[str]: Path to processed audio if successful, None otherwise
    """
    thread_prefix = f"{thread_id} " if thread_id else ""
    ...
    try:
        ...
        Logger.print_info(
            f"{thread_prefix}Processed audio saved to: {output_path}"
        )
        return output_path
        
    except subprocess.CalledProcessError as e:
        Logger.print_error(
            f"{thread_prefix}Error processing audio: {str(e)}"
        )
        return None
